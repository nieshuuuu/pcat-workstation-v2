<script lang="ts">
   ...
   * see the caveat banner. This replaces the 2-material water/lipid backing of
   * this viewer; the pericoronary ROI overlay still uses the self-calibrated GLS.
   *
   * Layout:
   *   ┌───────────────────────────────┬──────────────────────┐
   *   │                               │ Map: [f_w][f_l][f_p][σ]│
  ...
  // Refetch when the model (run/re-run) or slice index changes. The selected map
  // and CT-only toggle only re-render — no refetch.
  $effect(() => {
    void z;
    if (model) fetchSlice();
  });
